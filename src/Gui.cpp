...
void Gui::render()
{
  ...
  renderBar(1, 2, BAR_WIDTH, "MANA", 11, 20, TCODColor::lightBlue, TCODColor::darkerBlue);

  con->print(1, 3, "Damage: %g", engine.player->attacker->power);
  con->print(1, 4, "Defense: %g", engine.player->destructible->defense);
...
void Gui::renderBar(int x, int y, int width, const char *name, float value, float maxValue, const TCODColor &barColor, const TCODColor &backColor)
{
  ...
  if (value <= maxValue)
    barWidth = (int)(value / maxValue * width);
  else
    barWidth = (int)(value / (10 * maxValue) * width);
...
void Gui::renderMouseLook()
{
  ...
  for (Actor **it = engine.actors.begin(); it != engine.actors.end(); it++)
  {
    ...
    {
      if (engine.mouse.mbutton_pressed)
        return;
